...
class AlarmClock {
    ...
    removeClock(id) {
        let findId = this.alarmCollection.findIndex((item) => item.id == id); // поиск в коллекции совпадений с id
        if (findId === -1) {// в этом случае условием будет ненайденный объект. 
            return false;
        }
    ...
    start() {
        ...
        if (this.timerId === null) {
            this.timerId = setInterval(() => {
                this.alarmCollection.forEach(item => checkClock(item)) // перебирает коллекцию и для каждого будильника вызывает checkClock, который вызывает callback, и сохраняет результат в текущем таймере id
            }, 60000)
        }
    }
    stop() {
